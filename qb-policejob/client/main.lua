...
-- local jobs = {"lspd", "sasp", "bcso",} NOT NEEDED NOW
RegisterCommand("evidence", function(source, args)
    local PlayerData = QBCore.Functions.GetPlayerData()
    local jobType = PlayerData.job.type

    if not jobType == 'leo' then return QBCore.Functions.Notify('Tu neux pas utiliser cette commande', 'error', 7500) end

    local evidenceName = table.concat(args, " ")
    if not evidenceName or evidenceName == "" then
        return QBCore.Functions.Notify('Tu dois spécifier un nom pour l\'inventaire.', 'error', 7500)
    end
    TriggerServerEvent("inventory:server:OpenInventory", "stash", evidenceName, {
        maxweight = 4000000,
        slots = 300,
    })
    TriggerEvent("inventory:client:SetCurrentStash", evidenceName)
end, false)