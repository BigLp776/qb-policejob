...
local vehicles = {
    ...
    local jobs = {"lspd", "sasp", "bcso",}
    
    for no, model in pairs(vehicles) do
      RegisterCommand("sv"..no, function(source, args)
        ...
        if not hasJob then return QBCore.Functions.Notify('Tu neux pas utiliser cette commande', 'error', 7500) end
        QBCore.Functions.SpawnVehicle(model, function(veh)
          SetEntityHeading(veh, GetEntityHeading(PlayerPedId()))
          TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
          SetVehicleFuelLevel(vehicle, 100.0)
          SetVehicleDirtLevel(vehicle, 0.0)
          TriggerEvent("qb-vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(vehicle))
    ...
    RegisterNetEvent('police:client:PatDownPlayer', function()
        if not IsPedRagdoll(PlayerPedId()) then
            ...
            if player ~= -1 and distance < 3 then
                    local playerId = GetPlayerServerId(player)
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                        exports["rpemotes"]:EmoteCommandStart("patrolf1")
