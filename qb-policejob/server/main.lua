        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    exports['Renewed-Banking']:AddMoney('police', amount)
                else
...
QBCore.Commands.Add("frisk", "Palpation de la personne en face (Police Seulement)", {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if (Player.PlayerData.job.type == "leo") then
        TriggerClientEvent("police:client:PatDownPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)
...
RegisterNetEvent('police:server:PatDownPlayer', function(playerId)
    ...
    if TestedPlayer.PlayerData.money["cash"] >= 50000 then
     TriggerClientEvent('QBCore:Notify', src, "Tu remarque quelque chose de dur dans ses poches", "primary", 7500)
     TriggerClientEvent('QBCore:Notify', TestedPlayer.PlayerData.source, "Tu te fais palper", "primary", 7500)
    end
    for slot, item in pairs(TestedPlayer.PlayerData.items) do -- slot,
        if WeaponPatdownItems[item.name] then
            TriggerClientEvent('QBCore:Notify', src, "Tu sens une arme dans ses poches", 'success', 7500)
            TriggerClientEvent('QBCore:Notify', playerId, "Tu te fais palper", 'error', 7500)
        end
        if DrugPatdownItems[item.name] then
            TriggerClientEvent('QBCore:Notify', src, "Tu sens des sachets dans ses poches", 'success', 7500)
            TriggerClientEvent('QBCore:Notify', playerId, "Tu te fais palper", 'error', 7500)
        end
    end
end)
